...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for r in range(len(solution)):
        temp = set(get_row(solution, (r, 0)))
        if temp != set("123456789"):
            return False

    for c in range(len(solution)):
        temp = set(get_col(solution, (0, c)))
        if temp != set("123456789"):
            return False

    for r in (0, 3, 6):
        for c in (0, 3, 6):
            temp = set(get_block(solution, (r, c)))
            if temp != set("123456789"):
                return False
